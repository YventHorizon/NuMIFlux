...
h_pp_numu_dm = f.Get("ppnumu_dm_eleven")

cpp_numu_dm = TCanvas("c1","c1",1200,1200)

h_pp_numu_dm.SetTitle("#nu_{#mu} from #pi^{+}, #mu^{+} -> #bar{#nu}_{#mu} #nu_{e} e^{+}")
h_pp_numu_dm.GetXaxis().SetTitle("p_{#pi^{+}} [GeV/c]")
h_pp_numu_dm.GetXaxis().SetTitleFont(12)
h_pp_numu_dm.GetYaxis().SetTitle("#theta_{#pi^{+}} [mrad]")
h_pp_numu_dm.GetYaxis().SetTitleFont(12)
h_pp_numu_dm.GetXaxis().SetRangeUser(0,110)
h_pp_numu_dm.GetYaxis().SetRangeUser(0,500)
h_pp_numu_dm.Draw("colz")

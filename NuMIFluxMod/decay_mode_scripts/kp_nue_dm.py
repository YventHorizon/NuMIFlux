...
h_kp_nue_dm = f.Get("kpnue_dm_one")

ckp_nue_dm = TCanvas("c1","c1",1200,1200)

h_kp_nue_dm.SetTitle("#nu_{e} from #pi^{+}, #mu^{+} -> #bar{#nu}_{#mu} #nu_{e} e^{+}")
h_kp_nue_dm.GetXaxis().SetTitle("p_{#pi^{+}} [GeV/c]")
h_kp_nue_dm.GetXaxis().SetTitleFont(12)
h_kp_nue_dm.GetYaxis().SetTitle("#theta_{#pi^{+}} [mrad]")
h_kp_nue_dm.GetYaxis().SetTitleFont(12)
h_kp_nue_dm.GetXaxis().SetRangeUser(0,110)
h_kp_nue_dm.GetYaxis().SetRangeUser(0,500)
h_kp_nue_dm.Draw("colz")
